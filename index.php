...
<body>
    ...
    <div class="order-box" id="box">
        ...
        <div class="order-main-pane" Id='orderMP'>
            ...
            <form class="second-half-mainpane">
                <table>
                  <tr>
                    <td>
                      <p style=" font-family: 'Montserrat', cursive;  font-weight:bold">Your Name : </p>
                      <input type="text" class="address-box" id="namebox" requried style="margin-top:5px;font-size:100%" >
                    </td>
                    <td>
                      <p style=" font-family: 'Montserrat', cursive;  font-weight:bold">Your Contact No : </p>
                      <input type="text" class="address-box" id="concbox" requried style="margin-top:5px;font-size:100%" >
                    </td>
                  </tr>
                </table>
                
                
                <p style=" font-family: 'Montserrat', cursive;  font-weight:bold">Your Address : </p>
                <input type="text" class="address-box" id="addbox" requried style="margin-top:5px;font-size:100%" >
                
                <input type="radio" class="options" id="radiotakeaway" name="radioopt" value="Take away" style="position:static;display:inline">
                <label for="radiotakeaway">Take Away</label>
                <input type="radio" class="options" id="radiohomedel" name="radioopt" value="Home Delivery" style="position:static;display:inline">
                <label for="radiohomedel">Home Delivery</label> 
                <p style=" font-family: 'Montserrat', cursive;  font-weight:bold;display:block; margin:2px 10px;" id='total-lbl'></p>
                <button class='box-button' Id='order-box-submit' style=" font-family: 'Montserrat', cursive; display:block; margin: 20px auto 0 auto; ">Submit</button>
              </form>
        </div>
    </div>
        ...
        <section class='add-modification-box'>
            ...
            <p style=" font-family: 'Montserrat', cursive; font-weight:bold" >Quantity : </p>
                <input type="number" value=1 min=1 max=10 width="0" height="100%" id='mb-quan' required style=" font-family: 'Montserrat', cursive; ">
        ...
        <section class='login-regist-box'>
            ...
            <div class="log-rev-main">
                ...
                <p style=" font-family: 'Montserrat', cursive;  font-weight:bold">Username :  </p>
                    <input type="text" name="username" placeholder="Eg. James" required style=" font-family: 'Montserrat', cursive; " autofocus> 
            ...
            <?php
                    ...
                    if($result_check>0){
                        while($row = mysqli_fetch_assoc($result)){
                            ...
                            <div class='menu-item-name' id=".$row['item_price'].">
                               ".$row['item_name']."
                            </div>
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $("#order-box-submit").click(function(){
              //alert($("input[name='radioopt']:checked").val())
                var orderstatus = {
                custName: $('#namebox').val(),
                custConc: $('#concbox').val(),
                custAdd: $('#addbox').val(),
                custMode: $("input[name='radioopt']:checked").val(),
                custOrder: itemDetails,
              }
              //alert(orderstatus.custMode)
              console.log(orderstatus.custMode)
              var order=''
              var mods=''
              orderstatus.custOrder.forEach(item => {
                if(item.itemAdds!='')
                  mods=' with ' + item.itemAdds + ', '

                order=order + item.itemQuantity + ' ' + item.itemName + mods +'\n'
              })
              var outputAddMode=''
              //alert(typeof orderstatus.custMode)
              //alert(orderstatus.custMode)
              if(orderstatus.custMode==='Take away')
                outputAddMode = 'taken away by you'
              else if(orderstatus.custMode==='Home Delivery')
                outputAddMode = `delivered to ${orderstatus.custAdd} shortly`
                console.log(outputAddMode)
              //alert(outputAddMode)
              var outputString = `${orderstatus.custName} your Order of ${order} will be ${outputAddMode}`
              alert(outputString)
            ...
            $('#mb-add').click(function(){
                ...
                $('#orderLB').append(`<span class="order-name-box">${name}(${quan})<span class="order-name-cancel" 
                onclick="$(this).parent().css('display','none');var itemname=$(this).parent().text();itemname = (itemname.substring(0,itemname.indexOf('(')));itemDetails.splice(itemDetails.findIndex(ele => ele.itemName==itemname),1);"
                >x</span></span>`)
                calcOrder()
                //$('#total-lbl').text('Total : Rs. '+calcOrder())
            ...
            function deleteItem(itemname){
                ...
                /* var itemToBeDeleted = itemDetails.find(element => element.itemName==itemname)
                console.log(itemToBeDeleted) */
            }

            function calcOrder(){
              var total=0
              itemDetails.forEach(order => {
                console.log('order selected '+order.itemPrice)
                console.log('order selected '+order.itemQuantity)
                total=total+(Number(order.itemPrice)*Number(order.itemQuantity))
              })
              console.log(total)
              $('#total-lbl').text('Total : Rs. '+total)
              return total
